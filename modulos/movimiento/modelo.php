<?php
	...
	{   
    	...
    	{			    	
			...
			$option["select"]["IF(SUM(m1.orden)-SUM(m1.pago)>0, 'red','')"]="color1";
			$option["select"]["IF(SUM(m1.orden)-SUM(m1.pago)<0, 'green','')"]="color2";    
			$option["select"]["IF(SUM(m1.orden)-SUM(m1.pago)=0, '','')"]="color3";
