	...
	{   
    	...
    	{			 
            ...
            foreach($position_data as $row)
            {
            	...
            	$asunto			="";
            	if($row["speed_max"]>0 and $row["speed"]*1.852>=$row["speed_max"])		$asunto="VELOCIDAD";
            	else if($row["speed"]*1.852>=110)										$asunto="VELOCIDAD";
            	else
            	{
            	...
            	{		
            		...
            		$comando_sql		="";
            		$PRE_comando_sql	="INSERT INTO alert SET company_id	={$row["company_id"]}, fechaEvento	='{$row["devicetime"]}', ";
