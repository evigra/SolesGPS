	...
	foreach($travels_datas["data"] as $OV)
	{
		foreach($OV["movimientos_ids"] as $items)
		{
			...
			foreach($item_datas["data"] as $travel)
			{
				...
				{				
					$coord 						= explode(",",$punto);
					if($index==0)				$js.="origen=LatLng({latitude:{$coord[1]},longitude:{$coord[0]}});";
					else if($index==$total-2)	$js.="destino=LatLng({latitude:{$coord[1]},longitude:{$coord[0]}});";
					else if($index==$total-1)	$js.="tracert(origen,destino,waypts);";
					else
					{
						$js.="
							waypts.push({
