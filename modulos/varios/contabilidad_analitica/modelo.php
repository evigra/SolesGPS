...
*/			
		...
		public function reporte($option=NULL)
    	{
			...
			$option["select"]["round(monto,2)"] ="monto";
    	...
    	{
			$comando_sql="
				SELECT c.id, GROUP_CONCAT(cargo SEPARATOR '') as cargo,round(sum(cargo),2) as total_cargo, GROUP_CONCAT(abono  SEPARATOR '') as abono,round(sum(abono),2) as total_abono,type,
				...
				FROM (
					SELECT 
						...
						ca.name
					FROM admin_soles.contabilidad co JOIN admin_soles.contabilidad_analitica ca ON co.analitica_id=ca.id 
					ORDER BY id desc
				) c
				GROUP BY id	
			";			
